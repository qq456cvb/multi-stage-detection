...
class pascal_voc(imdb):
    ...
    def _load_image_set_index(self, cls_name=None):
        """
        ...
        image_set_file = os.path.join(self._data_path, 'ImageSets', 'Main',
                                      ('' if cls_name is None else cls_name + '_') + self._image_set + '.txt')
        ...
        with open(image_set_file) as f:
            if cls_name is None:
                ...
                image_index = [x.strip().split()[0] for x in f.readlines() if x.strip().split()[1] == '1']
        return image_index
